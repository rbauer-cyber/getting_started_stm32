...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Callback invoked for interrupts on PB4 and PB5
#if 0
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	// Prevent unused argument(s) compilation warning
	// Add code to use FastRotaryEncoder class
	if ( (GPIO_Pin == GPIO_PIN_5) || (GPIO_Pin == GPIO_PIN_4) )
	{
		FastRotaryEncoderChanged();
	}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	//const char notifyToggle[] = "STM32 toggling LED\n\r";
	// Timer 4 is on APB1 bus
	// Timer 4 is configured to run at 50 MHz
	// Default configuration interrupts every 20 nano-secs (reload count is 5000)
	//HAL_TIM_Base_Start_IT(&htim4);
	#if 0
	const char notifyStart[] = "STM32 blinky starting\n\r";
	HAL_Delay(1000);
